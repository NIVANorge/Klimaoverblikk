...
# Annual time series plot with one variable and 5-year moving average
ggplot(df_rivers_summ_a, aes(Year, DisTot/1E6, color = River_type)) +
  ...
  labs(x = "Year", y= "Discharge", color = "River type")

ggplot(df_rivers_summ_a, aes(Year, TrspTot_TOTN/1E6, color = River_type)) +
  geom_ma(ma_fun = SMA, n = 5, linetype = 1, size = 1) +
  geom_point() + 
  facet_grid(River_type~., scales = "free_y") +
  labs(x = "Year", y= "Transport TotN (/1E6)", color = "River type")

# NB! NO3 virker ikke
ggplot(df_rivers_summ_a, aes(Year, "TrspTot_NO3-N", color = River_type)) +
  geom_ma(ma_fun = SMA, n = 5, linetype = 1, size = 1) +
  geom_point() + 
  facet_grid(River_type~., scales = "free_y") +
  labs(x = "Year", y= "Transport TotN (/1E6)", color = "River type")

ggplot(df_rivers_summ_a, aes(Year, TrspTot_TOTP/1E6, color = River_type)) +
  geom_ma(ma_fun = SMA, n = 5, linetype = 1, size = 1) +
  geom_point() + 
  facet_grid(River_type~., scales = "free_y") +
  labs(x = "Year", y= "Transport TotP (/1E6)", color = "River type")

ggplot(df_rivers_summ_a, aes(Year, TrspTot_TOC/1E6, color = River_type)) +
  geom_ma(ma_fun = SMA, n = 5, linetype = 1, size = 1) +
  geom_point() + 
  facet_grid(River_type~., scales = "free_y") +
  labs(x = "Year", y= "Transport TOC (/1E6)", color = "River type")

# Annual time series plot with several variables for each river type (no sense in faceting because of the large difference - log??)
df_rivers_summ_a %>%
  gather("Variable", "Value"/1E6, TrspTot_TOTN:DisTot) %>%
...
df_rivers_summ_a %>%
  ...
  ggplot(aes(Year, Value)) +
    geom_ma(ma_fun = SMA, n = 5, linetype = 1, size = 1) + 
    geom_point() +
    facet_wrap(~Variable *River_type, scales = "free_y", labeller = label_both)
    labs(x = "Year", y= "Transports")

facet_wrap(~Variable *Rivertype)
    
# Distant - må fikse skalering
gg2 <- df_rivers_summ_a %>%
  ...
  ggplot(aes(Year, Value)) + 
    geom_point() +
   geom_ma(ma_fun = SMA, n = 5, linetype = 1, size = 1) +
    facet_grid(Variable~., scales = "free_y", , labeller = label_both)
gg2

# Local - må fikse skalering
gg2 <- df_rivers_summ_a %>%
  gather("Variable", "Value", TrspTot_TOTN:DisTot) %>%
...
### e. Plot flere varaibler i samme plot.
```{r}
str (df_hydro_summ_a)
...
# annual
df_hydro_summ_a %>%
  gather("Var", "Concentration", Temperatur, Salt) %>%
  ggplot(aes(Year, Concentration, color = Depth)) +
    geom_ma(ma_fun = SMA, n = 5, linetype = 1, size = 1) + 
    geom_point() +
    facet_grid(Var~., scales = "free_y", labeller = label_both)
