...
install.packages("sjPlot") # NB! denne krever relativt oppdatert versjon av R
library(sjPlot) # effectplots lmer
# library(pander)
#install.packages("glmulti",  lib = "H:/Documents/R/win-library/3.4")
#library(glmulti, lib.loc="H:/Documents/R/win-library/3.4")
#install.packages("rJava",  lib = "H:/Documents/R/win-library/3.4")
#library(rJava, lib.loc="H:/Documents/R/win-library/3.4")
...
### GLM models - subjective predictor choice
```{r}
str(dat_sel)

# Winter_NAO River__TOC

### 1a. Surface salinity
full_model <- glm(Salinity_Surface ~ River_Discharge + Winter_NAO, data = dat_sel, family = Gamma(link = "identity"))
#summary(full_model)
...
### GLM models - Automated predictor selection - NOT TESTED YET!
```{r}
str(dat_sel)
...
glmulti.POC <-
    glmulti(Hydro_POC_Surface ~ age + lwt + race.cat + smoke + preterm + ht + ui + ftv.cat, data = lbw,
            ...
            family = gamma)       # gamma family

## Show result for the best model
summary(glmulti.POC@objects[[1]])

#check also: library(bestglm)
```
