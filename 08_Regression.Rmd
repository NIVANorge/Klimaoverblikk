...
unique(dat.pca$Year) # mangler noe siden 2011
...
# legger inn i plot
plot(envvar$PC1, envvar$PC2)

envvarplot <- envvar %>% 
  ggplot(aes(PC1, PC2, label = row.names(envvar))) +
...
HBdat <- left_join(HBdf, dat)
names(HBdat)

DCAhb <- HBdat[, 3:6]
envHB <- HBdat[, 44:length(names(HBdat))]

envfitHB <- envfit(DCAhb, envHB, na.rm = TRUE, permutations = 999, choices = 1:4)
envfitHB # gives an indication of which parameters may be important for community responses along dca 1
str(envfitHB)
...
HBlmer.select <- step(HBlmer) # check envvar for missing values
