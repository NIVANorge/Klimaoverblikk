...
* HFR: annual plots of soft-bottom variables in script 01_ (section 4 c)
...
# NB! blir dette riktig?? får med 1989...
df_rivers_summ_q <- df %>%
...
# exclude local rivers and set 0 i transport to NA, se comment above
df_rivers_summ_a_t <- df_rivers_summ_a %>%
  filter(River_type != "Local") %>% 
  droplevels()%>% 
  mutate_all(funs(ifelse(. == 0, NA, .)))

df_rivers_summ_q_t <- df_rivers_summ_q %>%
  filter(River_type != "Local") %>% 
  droplevels() %>%
  mutate_all(funs(ifelse(. == 0, NA, .)))

# set transport of TOC in 1990 and 1991 to NA (only sampled in Glomma, too low total transports)
is.na (df_rivers_summ_a_t$TOC) <- 1:2

is.na (df_rivers_summ_q_t$TOC) <- 1:9
...
# HFR: NB! no scaling used here for now - need to discuss w/Øyvind
...
df_plank_summ_a %>%
  gather("Group", "Median", Kiselalger_med:Flagellater_med) %>%
