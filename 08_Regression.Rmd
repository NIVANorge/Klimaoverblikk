...
mod <- lm (Hydro_POC_Surface ~ 1, data = dat_sel)
plot(mod)
# many appear to be heteroscedadastic, SD increase with mean
# As I mentioned before, Toms suggested solution to this was Glm with gamma distribution.... any other suggestions?

```

### GLM models - Testing using glmulti
```{r}
str(dat_sel)
...
#### NB! Dag: diagnostics does not look very good... neither does effect plot.
...
########################## NB! models below are using stepAIC, which is probably not good for glms?
?stepAIC # Details: There is a potential problem in using glm fits with a variable scale, as in that case the deviance is not simply related to the maximized log-likelihood. The glm method for extractAIC makes the appropriate adjustment for a gaussian family, but may need to be amended for other cases. (The binomial and poisson families have fixed scale by default and do not correspond to a particular maximum-likelihood problem for variable scale.)

### 1a. Surface salinity
full_model <- glm(Hydro_Salinity_Surface ~ River_Discharge + Winter_NAO, data = dat_sel, family = Gamma(link = "identity"))
#summary(full_model)
step_model <- stepAIC(full_model, direction = "both", trace = FALSE)
summary(step_model) # only River_TOC...
par(mfrow = c(2,2), mar = c(4,5,2,1))
plot(step_model)
visreg(step_model, points = list(cex = 1))

### 1b. Deep salinity
full_model <- glm(Hydro_Salinity_Deep ~ River_Discharge + Winter_NAO, data = dat_sel, family = Gamma(link = "identity"))
#summary(full_model)
step_model <- stepAIC(full_model, direction = "both", trace = FALSE)
summary(step_model) # only River_TOC...
par(mfrow = c(2,2), mar = c(4,5,2,1))
...
# 2a. Surface TSM 
full_model <- glm(Hydro_TSM_Surface ~ River_TOC + River_SPM + Winter_NAO + Hydro_Salinity_Surface + Hydro_DIN_Surface + Hydro_PO4_Surface + Hydro_TotP_Surface + Hydro_TotP_Surface + Hydro_Chla_Surface + Hydro_Secchi_Surface, data = dat_sel, family = Gamma(link = "identity"))
summary(full_model)
step_model <- stepAIC(full_model, direction = "both", trace = FALSE)
summary(step_model) # River TOC and salinity surface higest significance
par(mfrow = c(2,2), mar = c(4,5,2,1))
plot(step_model)
visreg(step_model, points = list(cex = 1))
visreg(step_model, "River_TOC")

# 2b. deep TSM
full_model <- glm(Hydro_TSM_Deep ~ Hydro_TSM_Surface + River_TOC + River_SPM + Winter_NAO + Hydro_Salinity_Deep + Hydro_PO4_Deep + Hydro_TotP_Deep + Hydro_TotP_Surface + Hydro_Chla_Deep + Hydro_Secchi_Surface, data = dat_sel, family = Gamma(link = "identity"))
summary(full_model)
step_model <- stepAIC(full_model, direction = "both", trace = FALSE)
summary(step_model) # highest TSM_Surface
par(mfrow = c(2,2), mar = c(4,5,2,1))
plot(step_model)
visreg(step_model, points = list(cex = 1))

# 3a. Surface POC 
full_model <- glm(Hydro_POC_Surface ~ River_TOC + River_SPM + Winter_NAO + Hydro_Salinity_Surface + Hydro_DIN_Surface + Hydro_PO4_Surface + Hydro_TotP_Surface + Hydro_TotP_Surface + Hydro_Chla_Surface + Hydro_Secchi_Surface, data = dat_sel, family = Gamma(link = "identity"))
summary(full_model)
step_model <- stepAIC(full_model, direction = "both", trace = FALSE)
summary(step_model) # River TOC and salinity surface higest significance, also river_SPM
par(mfrow = c(2,2), mar = c(4,5,2,1))
plot(step_model)
visreg(step_model, points = list(cex = 1))
?stepAIC
...
# 3b.Intermediate POC
full_model <- glm(Hydro_POC_Intermediate ~ Hydro_POC_Surface + River_TOC + River_SPM + Winter_NAO + Hydro_Salinity_Intermediate + Hydro_DIN_Intermediate + Hydro_PO4_Intermediate + Hydro_TotP_Intermediate + Hydro_TotP_Intermediate + Hydro_Chla_Intermediate + Hydro_Secchi_Surface, data = dat_sel, family = Gamma(link = "identity"))
summary(full_model)
step_model <- stepAIC(full_model, direction = "both", trace = FALSE)
summary(step_model) # only River_TOC...
par(mfrow = c(2,2), mar = c(4,5,2,1))
...
# surface Chla 
full_model <- glm(Hydro_Chla_Surface ~ Hydro_Salinity_Surface + Hydro_DIN_Surface + Hydro_PO4_Surface + Hydro_Si_Surface + River_TOC + River_SPM + Winter_NAO + Hydro_TotP_Surface + Hydro_TotP_Surface + Hydro_Secchi_Surface, data = dat_sel, family = Gamma(link = "identity"))
summary(full_model)
step_model <- stepAIC(full_model, direction = "both", trace = FALSE)
summary(step_model) # highest POC_surface, also river_TOC
par(mfrow = c(2,2), mar = c(4,5,2,1))
plot(step_model)
visreg(step_model, points = list(cex = 1))
...
```{r}
### a. Surface salinity
full_model <- lm(Hydro_POC_Surface ~ River_TOC + River_SPM + Winter_NAO + Hydro_Salinity_Surface + Hydro_DIN_Surface + Hydro_PO4_Surface + Hydro_TotP_Surface + Hydro_TotN_Surface + Hydro_Chla_Surface + Hydro_Secchi_Surface, data = dat_sel) 
...
plot_model(HBlmer_best, sort.est = TRUE)
...
BB05_select <- stepAIC(BB05_lm, direction = "both", trace = 0) 
BB05_select

BB35_lm <- lm(sBB35$formula, sBB35$data)
BB35_select <- stepAIC(BB35_lm, direction = "both", trace = 0) 
BB35_select

# Modellplot BB05
plot_model(BB05_lm, type = "diag")
plot_model(BB05_lm, sort.est = TRUE)
ggsave ("Figures_rapp/ForestEffect_BB05DCA.png", width = 6, height = 4, dpi=500)

# Modellplot BB35
plot_model(BB35_lm, type = "diag")
plot_model(BB35_lm, sort.est = TRUE)
...
```{r effect plot, fig.height = 10}
plot_model(BB05_lm, type = "slope", show.data = TRUE)
ggsave ("Figures_rapp/ForestEffect_BB05DCA.png", width = 10, height = 10, dpi=500)

plot_model(BB35_lm, type = "slope", show.data = TRUE)
ggsave ("Figures_rapp/ForestEffect_BB05DCA.png", width = 10, height = 10, dpi=500)
```
